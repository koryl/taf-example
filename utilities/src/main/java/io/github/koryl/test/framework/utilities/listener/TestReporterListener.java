...
public class TestReporterListener implements IReporter {
    ...
    private static final String ROW_TEMPLATE = "<tr class=\"%s\"><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>";
    private static final String ROW_FAIL_TEMPLATE = "<tr class=\"clickable %s\" data-toggle=\"collapse\" id=\"%s\" data-target=\"%s\"><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>";
    private static final String ROW_IMAGE = "<tr class=\"collapse %s\"><td>-</td><td colspan=\"2\">%s</td><td colspan=\"3\">%s</td></tr>";
        ...
        return testResult -> {
            switch (testResult.getStatus()) {
                case ITestResult.FAILURE:
                    return getImageCollapse(testName, suiteName, testResult);
    ...
    private String getImageCollapse(String testName, String suiteName, ITestResult testResult) {
        String randomId = String.valueOf(RandomUtils.nextInt());
        String row = String.format(ROW_FAIL_TEMPLATE, "danger", randomId, "." + randomId, "<i class=\"glyphicon glyphicon-plus\"></i>", suiteName, testName, testResult.getName(), "FAILED", "NA");
        List<String> output = Reporter.getOutput(testResult);
        if(output.size() != 0) {
            String imageRow = String.format(ROW_IMAGE, randomId, output.get(0), output.get(1));
            return row + imageRow;
        } else {
        return row;
        }
    ...
    private void saveReportTemplate(String outputDirectory, String reportTemplate) {
        ...
        try {
            PrintWriter reportWriter = new PrintWriter(new BufferedWriter(new FileWriter(new File(outputDirectory, "custom-report.html"))));
